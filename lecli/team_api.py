...
def create_team(name):
    ...
    try:
        ...
        if response_error(response):
            print "Creating team failed, status code: %s" % str(response.status_code)
            exit(1)
...
def delete_team(team_id):
    ...
    try:
        ...
        if response_error(response):  # Check response has no errors
            print 'Delete team failed, status code: ' + str(response.status_code)
            exit(1)
...
def rename_team(team_id, team_name):
    ...
    try:
        ...
        if response_error(response):  # Check response has no errors
            print 'Renaming team with id: %s failed, status code: %s' \
                  % (team_id, str(response.status_code))
            exit(1)
        elif response.status_code == 200:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                response = requests.patch(url, json=params, headers=headers)
                if response_error(response):  # Check response has no errors
                    print 'Adding user to team with id: %s failed, status code: %s' \
                          % (team_id, str(response.status_code))
                    exit(1)
                elif response.status_code == 200:
            ...
            print 'Adding user to team %s failed, status code: %s' % (team_id,
